...
_routes = importlib.import_module("p5_ops.1_control_panel.1_routes")
register_enhanced_routes = _routes.register_enhanced_routes
...
async def list_voice_profiles() -> Any:
    ...
    try:
        from serin.gateway.voice_transcribe.profiles import get_voice_profiles, get_active_profile_name
...
async def create_voice_profile(data: Dict[str, Any]) -> Any:
    ...
    try:
        from serin.gateway.voice_transcribe.profiles import create_profile
        name = data.get('name')
        if not name:
...
async def set_active_voice_profile(profile_name: str = 'default') -> Any:
    ...
    try:
        from serin.gateway.voice_transcribe.profiles import set_active_profile
...
async def delete_voice_profile(profile_name: str) -> Any:
    ...
    try:
        from serin.gateway.voice_transcribe.profiles import delete_profile
