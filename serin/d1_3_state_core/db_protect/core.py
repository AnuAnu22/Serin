...
class DatabaseProtectorCore:
    def __init__(self, data_dir: str = "./bot_data", backup_dir: str | None = None) -> None:
        ...
        self.backup_count = 0
        self.validation_failures: list[Any] = []
    ...
    def validate_all_databases(self) -> dict[str, Any]:
        validation_start: float = time.time()
    ...
    def _validate_sqlite_database(self) -> dict[str, Any]:
        results: dict[str, Any] = {
            'valid': False,
            'exists': False,
            'readable': False,
        ...
        try:
            ...
            conn: sqlite3.Connection = sqlite3.connect(str(self.sqlite_db))
            try:
                cursor: sqlite3.Cursor = conn.cursor()
                cursor.execute("PRAGMA integrity_check;")
                integrity_row: sqlite3.Row | None = cursor.fetchone()
                if integrity_row is None:
                    results['error'] = "SQLite integrity check returned no result"
                    return results
                ...
                page_row: sqlite3.Row | None = cursor.fetchone()
                if page_row is not None:
                    results['page_count'] = page_row[0]
    ...
    def _validate_chroma_database(self) -> dict[str, Any]:
        results: dict[str, Any] = {
            'valid': False,
...
_database_protector: DatabaseProtectorCore | None = None


def get_database_protector() -> DatabaseProtectorCore:
