...
def _condense_results(store: 'QdrantMemorySystem', results: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        Keeps the full per-candidate scoring breakdown (``score_breakdown``)
        alongside the flattened ``relevance`` figure. The breakdown is what the
        control panel's memory browser renders as the "point system" for each
        result — nothing here is derived data; every field already existed on
        the pipeline's intermediate ``result`` dict, it was just being dropped
        on the floor before this function returns.
        """
        condensed = []
        ...
        for result in results:
            ...
            rerank_score = result.get('rerank_score', combined_score)
            importance = payload.get('importance', 0.5)
            ...
            condensed.append({
                ...
                'score_breakdown': {
                    ...
                    'importance_weight': round(float(importance), 4),
                    'recency_boost': round(float(recency_boost), 4),
                    'formula': 'combined = vector*0.6 + (1/(1+bm25))*0.4; '
                               'rerank = combined + recency_boost*0.2 + (importance-0.5)*0.1',
                },
            })
