...
class VoiceListener:
    def __init__(self, client: discord.Client, audio_processor: Any) -> None:
        ...
        get_logger().info("Voice listener initialized (InfoCaptureProtocol + Rust)")

    async def join_channel(self, guild_id: int, channel_id: int) -> bool:
        if guild_id in self._join_in_progress:
            get_logger().warning(f"Join already in progress for guild {guild_id}, skipping")
            return False
        self._join_in_progress.add(guild_id)
        try:
            guild = self.client.get_guild(guild_id)
            if not guild:
                get_logger().error(f"Guild {guild_id} not found")
                return False

            channel = guild.get_channel(channel_id)
            if not channel or not isinstance(channel, discord.VoiceChannel):
                get_logger().error(f"Voice channel {channel_id} not found")
                return False
            ...
            try:
                ...
                get_logger().error("Timeout waiting for voice connection info from gateway")
                return False
            ...
            from serin.d1_2_gateway_io.d2_2_voice_system.bridge import RustVoiceBridge
            ...
            if not success:
                get_logger().error("Failed to start Rust voice receiver")
            ...
            get_logger().info(f"Joined {channel.name} in {guild.name} (Rust owns voice)")
            return True

        except Exception as e:
            get_logger().exception(f"Error joining voice channel: {e}")
            self.stats['errors'] += 1
            return False
        finally:
    ...
    async def leave_channel(self, guild_id: int) -> bool:
        try:
            ...
            self._join_in_progress.discard(guild_id)
            get_logger().info(f"Left voice channel in guild {guild_id}")
            return True

        except Exception as e:
            get_logger().error(f"Error leaving voice channel: {e}")
            self.stats['errors'] += 1
            return False
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        if self.rust_bridge:
            try:
                ...
                get_logger().exception("Failed to get Rust bridge stats")
        return stats
