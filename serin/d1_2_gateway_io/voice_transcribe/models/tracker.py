...
from serin.d1_4_config_base.debug_logger import log_voice
...
class VoiceTracker:
    ...
    def __init__(self, memory_system: Any) -> None:
        ...
        get_logger().info(" Voice tracker initialized")
        ...
        try:
            ...
            # Muted/unmuted, deafened/undeafened (track but don't store)
                ...
                if before.self_mute != after.self_mute:
                    ...
                    get_logger().debug(f" {username} {action}")
            ...
            get_logger().error(f" Error handling voice update: {e}")
    ...
    async def _handle_join(self, member: Any, channel: Any) -> None:
        ...
        get_logger().info(f" {username} joined VC: {channel_name}")
    ...
    async def _handle_leave(self, member: Any, channel: Any) -> None:
        ...
        if user_id in self.session_start_times:
            ...
            get_logger().info(f" {username} left VC after {duration_minutes} min")
        else:
            get_logger().warning(f" {username} left VC but no session start time found")
    ...
    async def _handle_switch(self, member: Any, old_channel: Any, new_channel: Any) -> None:
        ...
        get_logger().info(f" {username} switched: {old_name} → {new_name}")
