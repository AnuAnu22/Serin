...
class WhisperTranscriber:
    def __init__(self, model_size: str = "base", device: str = "cuda", compute_type: str = "float16") -> None:
        ...
        if not WHISPER_AVAILABLE:
            get_logger().error(" faster-whisper not available")
            return

        get_logger().info(" Whisper transcriber initialized")
        get_logger().info(f"    Model: {model_size}")
        get_logger().info(f"    Device: {device.upper()}")
        get_logger().info(f"    Compute: {self.compute_type}")

    async def load_model(self) -> bool:
        """Load Whisper model"""
        if not WHISPER_AVAILABLE:
            get_logger().error(" Cannot load Whisper model - faster-whisper not installed")
            return False

        try:
            get_logger().info(f" Loading Whisper model '{self.model_size}'...")
            ...
            get_logger().info(f" Whisper model '{self.model_size}' loaded")
            return True

        except Exception as e:
            get_logger().error(f" Error loading Whisper model: {e}")
            self.stats['errors'] += 1
            return False
    ...
    async def transcribe(self, audio_data: bytes, language: str = "en") -> str | None:
        ...
        """
        if not self.model:
            get_logger().error(" Whisper model not loaded")
            return None
        ...
        try:
            ...
            get_logger().debug(" Transcribing audio...")
            ...
            if transcription:
                get_logger().info(f" Transcription: '{transcription}'")
                return transcription
            else:
                get_logger().debug(" Empty transcription")
                return None

        except Exception as e:
            get_logger().error(f" Error transcribing audio: {e}")
            self.stats['errors'] += 1
            return None
