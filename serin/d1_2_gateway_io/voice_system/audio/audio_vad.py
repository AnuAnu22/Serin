...
def _queue_for_transcription(
    ...
    try:
        ...
        if not buffer or len(buffer) < 192000:
            get_logger().debug(f" Skipping empty/short buffer for {username}")
        ...
        try:
            ...
            get_logger().debug(f" Queued {len(audio_data)} bytes for transcription: {username}")

        except asyncio.QueueFull:
            get_logger().warning(f" Transcription queue full, dropping audio from {username}")
    ...
    async def _timer() -> None:
        ...
        if user_id in self.user_buffers and len(self.user_buffers[user_id]) > 0:
            if user_id in self.currently_speaking:
                get_logger().debug(f"[DBG-VAD] Silence timer fired for {username} ({len(self.user_buffers[user_id])}B) — queueing transcription")
...
async def _process_queue(self) -> None:
    ...
    while self.is_running:
        try:
            ...
            get_logger().info(" Transcription queue processor cancelled")
            break
        except Exception as e:
            get_logger().error(f" Error in transcription queue: {e}")
            self.stats['errors'] += 1
            await asyncio.sleep(0.5)

    get_logger().info(" Transcription queue processor stopped")
