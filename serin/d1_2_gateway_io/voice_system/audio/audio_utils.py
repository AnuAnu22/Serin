...
async def _transcribe_with_gemma(self: Any, audio_data: bytes, username: str = "User") -> str | None:
    ...
    try:
        ...
        if len(audio_data) > max_audio_bytes:
            get_logger().info(f" Truncating audio from {len(audio_data)} to {max_audio_bytes} bytes (30s limit)")
...
def process_audio_chunk(
    ...
    try:
        ...
        if user_id in self.currently_speaking and self.stats['chunks_received'] % 500 == 0:
            ...
            get_logger().debug(f"[DBG-VAD] user={user_id} speaking buf={buf_size}B silence_frames={self.user_silence_frames.get(user_id, 0)}")
        ...
        if is_voice:
            ...
            if len(self.user_buffers[user_id]) >= self.MAX_BUFFER_BYTES:
                get_logger().debug(f"[DBG-VAD] Buffer overflow ({len(self.user_buffers[user_id])}B) — forcing transcription")
            ...
            if user_id not in self.currently_speaking:
                ...
                get_logger().debug(f" {username} started speaking")
                ...
                if self.voice_output_manager:
                    try:
                        ...
                        get_logger().debug(f"Could not interrupt TTS: {e}")
        ...
        get_logger().error(f" Error processing audio chunk: {e}")
        self.stats['errors'] += 1
