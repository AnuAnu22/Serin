...
def _handle_process_death(self: RustVoiceBridge) -> None:
    ...
    if exit_code is None:
        ...
        get_logger().error("voice.process_killed", extra={
            "exit_code": exit_code,
            "signal": signal_name,
            "guild_id": str(self._guild_id),
        })
    else:
        get_logger().error("voice.process_exited", extra={
            "exit_code": exit_code,
            "guild_id": str(self._guild_id),
        })
...
async def _supervise_rust_process(self: RustVoiceBridge) -> None:
    ...
    while not self._shutdown_requested:
        ...
        if len(self._restart_timestamps) >= 5:
            ...
            if now - oldest < 60.0:
                get_logger().critical("voice.supervisor_giving_up", extra={
        ...
        get_logger().warning("voice.process_restarting", extra={
        ...
        if guild_id is None or channel_id is None:
            get_logger().error("Guild or channel ID missing — cannot restart")
            return
        try:
            ...
            get_logger().exception(f"Failed to restart Rust process: {e}")

        if success:
            get_logger().info("Rust voice process restarted successfully")
            if self._reconnect_callback:
                try:
                    await self._reconnect_callback()
                except Exception as e:
                    get_logger().error(f"Reconnect callback failed: {e}")
            # Clear death event so supervisor waits for next death
            self._death_event.clear()
        else:
            get_logger().error("Failed to restart Rust process — will retry")
        ...
        try:
            ...
            for line in stderr_text:
                ...
                if any(kw in line for kw in ['ERROR', 'JOIN_FAILED', 'PANIC']):
                    get_logger().error(f"   [rust] {line}")
                elif any(kw in line for kw in ['CONNECTED', 'READY', 'GOT_INFO']):
                    get_logger().info(f"   [rust] {line}")
                elif 'RTP' in line or 'SPEAKING' in line:
                    get_logger().debug(f"   [rust] {line}")
                else:
                    get_logger().debug(f"   [rust] {line}")
        except Exception as e:
            get_logger().debug(f"   [stderr reader] exited: {e}")
