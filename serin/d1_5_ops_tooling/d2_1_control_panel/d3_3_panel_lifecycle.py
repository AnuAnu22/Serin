...
async def start_server(app: FastAPI | None = None, host: str = "127.0.0.1", port: int = 8080) -> None:
    ...
    if host != "127.0.0.1" and not config.CONTROL_PANEL_KEY:
        # This used to be a warning only. A control panel that can restart
        # the bot process, rewrite LLM_BASE_URL, and read every stored
        # memory has no business being reachable on the network with no
        # auth at all — refuse to start rather than silently expose it.
        raise RuntimeError(
            "CONTROL_PANEL_KEY must be set before binding the control panel "
            f"to a non-local host ({host}). Set CONTROL_PANEL_KEY in .env, "
            "or bind to 127.0.0.1 and use SSH port-forwarding for remote access."
        )
    max_retries = 5
    current_port = port
